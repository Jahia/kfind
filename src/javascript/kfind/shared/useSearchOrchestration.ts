...
export const useSearchOrchestration = (
  ...
  const triggerAvailabilityChecks = useCallback(() => {
    if (availabilityTriggeredRef.current) return;
  ...
  const executeSearch = useCallback(
    (query: string) => {
      ...
      if (!stateRef.current.availabilityResolved) return;
  ...
  const resetAll = useCallback(() => {
    for (const d of drivers) d.provider.reset();
  ...
  useEffect(() => {
    if (!state.availabilityResolved) return;
  ...
  const activeDrivers: SearchOrchestrationResult["drivers"] = useMemo(() => {
    return drivers
      .filter((d) => {
        ...
        if (!d.registration.isEnabled()) return false;
      ...
      .map((d) => ({
        ...
        loadNextPage: () => {
          ...
          if (!ds || ds.loading || !ds.hasMore) return;
