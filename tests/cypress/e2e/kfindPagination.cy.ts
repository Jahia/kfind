...
describe('kFind pagination behavior', () => {
    ...
    it('shows a Show more button when a section has additional results', () => {
        ...
        cy.get(SHOW_MORE_SELECTOR, {timeout: 20000}).first().should('be.visible');
    });

    it('loads more results after clicking Show more', () => {
        searchInModal(`kfind pagination ${token}`);

        cy.get(RESULT_ROW_SELECTOR, {timeout: 20000})
            .its('length')
            .then(initialCount => {
                const countBefore = Number(initialCount);
                expect(countBefore).to.be.greaterThan(0);

                cy.get(SHOW_MORE_SELECTOR, {timeout: 20000}).first().click();

                cy.get(RESULT_ROW_SELECTOR, {timeout: 20000}).its('length').should('be.greaterThan', countBefore);
            });
    });
    ...
    it('keeps focus on Show more when pressing Arrow keys', () => {
        ...
        cy.get(SHOW_MORE_SELECTOR, {timeout: 20000}).first().focus();
